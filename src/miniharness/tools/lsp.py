...
from miniharness.services.lsp import (
    find_references,
    go_to_definition,
    hover,
...
class LspInput(BaseModel):
    ...
    def validate_arguments(self) -> "LspInput":
...
class LspTool(BaseTool):
    ...
    def is_read_only(self, arguments: LspInput) -> bool:
        del arguments
        return True

    def permission_requests(self, arguments: LspInput) -> list[ToolPermissionRequest]:
        if arguments.operation == "workspace_symbol":
            return [ToolPermissionRequest(is_read_only=True, file_path=str(self.cwd))]
        if arguments.file_path:
            return [ToolPermissionRequest(is_read_only=True, file_path=arguments.file_path)]
        return []

    async def execute(self, arguments: LspInput) -> ToolResult:
